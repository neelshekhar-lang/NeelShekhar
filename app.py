...
m1.metric(f"Weight: {name1}", f"{w_opt:.1%}", delta=f"{w_opt - 0.5:.1%} vs 50/50")
m2.metric(f"Weight: {name2}", f"{1 - w_opt:.1%}", delta=f"{(1-w_opt) - 0.5:.1%} vs 50/50")
m3.metric("Expected Return", f"{er_opt:.2%}", delta=f"{er_opt - rf:.2%} excess return")
m4.metric("Std Dev (Risk)", f"{sd_opt:.2%}")
m5.metric("Sharpe Ratio", f"{sharpe_opt:.3f}", delta=f"{sharpe_opt - sharpe_max:.3f} vs max Sharpe")
m6.metric("ESG Score", f"{esg_opt:.3f}", delta=f"{esg_opt - esg_sharpe:.3f} vs max Sharpe ptf")
...
df_summary = pd.DataFrame({
    ...
    "Weight in Asset 1": ["100%", "0%", f"{w_opt:.1%}", f"{w_sharpe:.1%}"],
    "Weight in Asset 2": ["0%", "100%", f"{1-w_opt:.1%}", f"{1-w_sharpe:.1%}"],
...
ax.plot(sd_p * 100, er_p * 100, color=DARK_GREEN, lw=2.5, label="Mean-Variance Frontier", zorder=2)

if sharpe_max > 0:
    cml_x = np.linspace(0, max(sd_p) * 1.1, 200)
    cml_y = rf * 100 + sharpe_max * cml_x
    ax.plot(cml_x, cml_y, '--', color=GREY_BENCH, lw=1.5, alpha=0.7, label="Capital Market Line")

ax.scatter([sd1 * 100], [er1 * 100], color=BLUE_ASSET, s=120, zorder=5, label=name1, marker='D')
ax.scatter([sd2 * 100], [er2 * 100], color=ORG_ASSET,  s=120, zorder=5, label=name2, marker='D')
ax.scatter([sd_p[sharpe_idx]*100], [er_p[sharpe_idx]*100], color=GREY_BENCH, s=110, zorder=5, marker='s', label="Max-Sharpe Portfolio")
ax.scatter([sd_opt * 100], [er_opt * 100], color=GOLD_OPT, s=220, zorder=6, marker='*', edgecolors='black', linewidths=0.5, label="★ Optimal Portfolio (Your Choice)")
...
ax2.plot(esg_targets[valid], frontier_sharpe[valid], color=DARK_GREEN, lw=2.5, label="ESG-Efficient Frontier\n(max Sharpe at each ESG level)", zorder=2)
ax2.scatter([esg1], [sharpe1], color=BLUE_ASSET, s=120, zorder=5, label=name1, marker='D')
ax2.scatter([esg2], [sharpe2], color=ORG_ASSET,  s=120, zorder=5, label=name2, marker='D')
ax2.scatter([esg_sharpe], [sharpe_max], color=GREY_BENCH, s=110, zorder=5, marker='s', label="Max-Sharpe Portfolio")
ax2.scatter([esg_opt], [sharpe_opt], color=GOLD_OPT, s=220, zorder=6, marker='*', edgecolors='black', linewidths=0.5, label="★ Optimal Portfolio (Your Choice)")

if esg_cost > 0.002 and lambda_ > 0:
    ax2.annotate("", xy=(esg_opt, sharpe_opt), xytext=(esg_sharpe, sharpe_max),
        arrowprops=dict(arrowstyle="<->", color="red", lw=1.5))
    mid_esg = (esg_opt + esg_sharpe) / 2
    mid_sr  = (sharpe_opt + sharpe_max) / 2
    ax2.text(mid_esg + 0.01, mid_sr, f"ESG cost\n−{esg_cost:.3f} SR", fontsize=8, color="red")
